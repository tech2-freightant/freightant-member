...
export async function GET(request: NextRequest) {
    try {
      ...
      const query = "SELECT DISTINCT `t1`.`id`, `t1`.`Country`, `t1`.`Name`,`t1`.`Location`,`t1`.`Subdivision`,`t2`.`emoji`, `t2`.`name` AS `countryname`, `t1`.`FullName` FROM `locode` AS `t1` INNER JOIN `countries` AS `t2` ON `t2`.`iso2` = `t1`.`Country` WHERE `t1`.`Location` LIKE '%"+name.get("name")+"%' OR `t1`.`Country` LIKE '%"+name.get("name")+"%' OR `t1`.`Name` LIKE '%"+name.get("name")+"%' OR `t1`.`FullName` LIKE '%"+name.get('name')+"%' LIMIT 50";
